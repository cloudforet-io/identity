...
from spaceone.identity.model.domain_secret.database import DomainSecret
...
class DomainSecretManager(BaseManager):
    ...
    def create_domain_secret(self, domain_id: str) -> None:
        def _rollback(vo):
            _LOGGER.info(
    ...
    def get_domain_refresh_public_key(self, domain_id):
        ...
        if not domain_secret_vo.refresh_pub_jwk:
            domain_secret_vo: DomainSecret = self._create_refresh_key(
                domain_secret_vo, domain_id
            )
    ...
    def _create_refresh_key(self, domain_secret_vo: DomainSecret, domain_id):
        domain_vo = self.domain_model.get(domain_id=domain_id)
