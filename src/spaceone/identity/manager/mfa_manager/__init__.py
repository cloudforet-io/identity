...
class BaseMFAManager(BaseManager, ABC):
    ...
    def _load_conf(self):
        ...
        self.CONST_MFA_VERIFICATION_CODE_TIMEOUT = mfa_conf.get('mfa_verify_code_timeout', 300)
...
class MFAManager(BaseMFAManager, metaclass=ABCMeta):
    ...
    def create_mfa_verify_code(self, user_id, domain_id):
        if cache.is_set():
            ...
            cache.set(f'mfa-verify-code:{domain_id}:{user_id}', verify_code, expire=self.CONST_MFA_VERIFICATION_CODE_TIMEOUT)
            return verify_code
