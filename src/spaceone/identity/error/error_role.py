...
class ERROR_NOT_ALLOWED_ROLE_TYPE(ERROR_INVALID_ARGUMENT):
    _message = (
        "Duplicate assignment of system role and domain or project role is not allowed."
    )
...
class ERROR_DUPLICATE_ROLE_BOUND(ERROR_INVALID_ARGUMENT):
    _message = (
        "Duplicate role bound. (role_id = {role_id}, resource_id = {resource_id})"
    )
...
class ERROR_POLICY_IS_IN_USE(ERROR_INVALID_ARGUMENT):
    _message = (
        "The policy is in use by role. (policy_id = {policy_id}, role_id = {role_id})"
    )
